...
float PuissanceS_T_moy, PuissanceS_M_moy, PuissanceI_T_moy, PuissanceI_M_moy;
float PVAS_T_moy, PVAS_M_moy, PVAI_T_moy, PVAI_M_moy;
...
void GestionOverproduction() {
  ...
  for (int i = 0; i < NbActions; i++) {
    Actif[i] = LesActions[i].Actif;                                                       //En RAM
    ...
    if (Actif[i] > 0 && Type_En_Cours > 1 && DATEvalid) {  // On ne traite plus le NO
      if (Type_En_Cours == 2) {
        ...
        RetardF[i] = RetardF[i] + 0.001;                                       //On ferme très légèrement si pas de message reçu
        ...
        if (i == 0) {  // le Triac
          ...
          if (ITmode < 0) RetardF[i] = 100;                                   //Pas possible sur synchro interne
        } else {                                                              //Les relais
          if (Actif[i] == 1 && Puissance > MaxTriacPw) { RetardF[i] = 100; }  //OFF
          if ((RetardF[i] < 100 - MaxTriacPw) && Actif[i]>1 ) { RetardF[i] = 100 - MaxTriacPw; }
        }
